...
function buildAccumulators(preds: Prediction[]) {
  // ensure one prediction per match (use first occurrence for matchId)
  ...
  for (const size of sizes) {
    for (let i = 0; i + size <= list.length; i++) {
      accs.push(list.slice(i, i + size));
    }
  }
  // dedupe by join of ids
        ...
        <>
          {accumulators.length > 0 && (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
              {accumulators.map((group, idx) => (
                <AccumulatorCard key={group.map(g => g.id).join('-')} predictions={group} title={`${group.length}-Match VIP Accumulator`} />
              ))}
            </div>
          )}
