...
const PredictionCarousel = ({ title, predictions, icon: Icon, link, isLoading }: { title: string; predictions: Match[]; icon: React.ElementType; link: string, isLoading: boolean; }) => {
  if (isLoading) {
    return (
    ...
    <Card className="shadow-lg border-none">
      ...
      <CardContent>
        <div className="flex justify-center items-center h-40 bg-muted/50 rounded-lg">
            <p className="text-muted-foreground">View all upcoming matches on the dedicated page.</p>
        </div>
      </CardContent>
    </Card>
  );
  ...
  return (
    <div className="space-y-12">
      <div className="grid grid-cols-1 gap-8 lg:grid-cols-2">
        <DashboardCard title="Upcoming Matches" icon={Calendar} link="/matches" />
