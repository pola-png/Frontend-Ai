...
const getSafeKey = (p: Prediction, index: number) =>
  p.id || p.matchId || `${p.bucket}-${index}`;
      ...
      <div className="flex flex-col h-full">
        ...
        <CardContent>
          {isLoading ? (
            ...
            <Carousel
              opts={{ align: 'start', loop: predictions.length > 2 }}
              className="w-full"
            >
              <CarouselContent className="-ml-4">
                {predictions.map((p, i) => (
  ...
  useEffect(() => {
    const fetchData = async (
      ...
      try {
        ...
        console.error(`Failed to fetch ${bucket} predictions`, err);
      } finally {
        setLoading(false);
      }
    };

    fetchData('vip', setVipPredictions, setLoadingVip);
    fetchData('daily2', setTwoOddsPredictions, setLoadingTwoOdds);
    fetchData('value5', setFiveOddsPredictions, setLoadingFiveOdds);
    fetchData('big10', setBigOddsPredictions, setLoadingBigOdds);
  }, []);
