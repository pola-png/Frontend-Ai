...
type MatchInfoCardProps = { match: Match };
    ...
    <Card className="flex flex-col h-full bg-card shadow-md hover:shadow-xl transition-shadow duration-300 border-border/20">
      ...
      <CardContent className="flex-1 flex flex-col justify-center items-center space-y-4">
        <div className="flex w-full items-center justify-between text-center">
          ...
          <div className="w-1/5"><span className="text-xl font-bold text-muted-foreground">vs</span></div>
          <div className="flex flex-col items-center gap-2 w-2/5">
            <Avatar>
        ...
        {predictions && predictions.length > 0 && (
          <div className="pt-4 w-full">
            <h4 className="text-sm font-semibold mb-2 text-center text-primary">
              AI Predictions - Total Odds: {totalOdds?.toFixed(2)}
            </h4>
              ...
              <div className="flex flex-wrap justify-center gap-2">
                {predictions.map(p => (
                  <Badge key={p.id} variant="secondary" className="flex items-center gap-1">
                    <ShieldCheck className="h-3 w-3 text-green-500" /> {p.prediction} ({p.odds?.toFixed(2)})
                  </Badge>
                ))}
              </div>
            )}
            <button
              ...
              className="flex items-center gap-1 text-xs text-primary font-medium mt-2 mx-auto"
            >
